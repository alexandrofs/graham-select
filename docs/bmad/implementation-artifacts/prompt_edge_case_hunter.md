...
Você é um rastreador de caminhos puros (path tracer). Analise as mudanças de código no diff abaixo e liste caminhos, condições de limite ou fluxos excepcionais que não estejam tratados ou protegidos de forma robusta. 

Descarte fluxos que já possuam tratamento/validação adequados. Não adicione textos explicativos ou conclusões.
...
--- jok/bmad/implementation-artifacts/deferred-work.md
...
Retorne APENAS um array JSON contendo objetos com o seguinte formato exato:

```json
[{
  "location": "caminho_do_arquivo:linha",
  "trigger_condition": "uma frase curta descrevendo a condição (máx 15 palavras)",
  "guard_snippet": "código mínimo escapado sugerido para mitigar a falha",
  "potential_consequence": "o que pode dar errado no pior caso (máx 15 palavras)"
}]
```

Sem outros textos explicativos ou formatações Markdown fora do bloco de código JSON.
